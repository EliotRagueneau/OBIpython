...
def find_orf(seq: str, threshold: int, code_table_id: int):
    ...
    print(len(stop_lidi), len(start_lidi))
    for i in range(len(stop_lidi)):
        for j in range(i + 1, len(stop_lidi)):
            if stop_lidi[i]["frame"] == stop_lidi[j]["frame"]:

                for k in range(len(start_lidi)):

                    if stop_lidi[j]["frame"] > 0:
                        # print(2135465464)
                        print(stop_lidi[j]["pos"] - start_lidi[k]["pos"])
                        if start_lidi[k]["frame"] == stop_lidi[i]["frame"] and \
                                stop_lidi[j]["pos"] - start_lidi[k]["pos"] > threshold:
                            # print(3165464646464)
